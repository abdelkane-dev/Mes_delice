...
const NotificationsAPI = {
    ...
    // GET /api/notifications/recent/ - 20 dernières notifications
    getRecent: async () => {
        return await apiCall(`${API_ENDPOINTS.notifications}recent/`);
    },

    // GET /api/notifications/unread_count/ - Nombre de non lues
    getUnreadCount: async () => {
        return await apiCall(`${API_ENDPOINTS.notifications}unread_count/`);
    },

    // POST /api/notifications/{id}/mark_as_read/ - Marquer comme lue
    markAsRead: async (id) => {
        return await apiCall(`${API_ENDPOINTS.notifications}${id}/mark_as_read/`, {
            method: 'POST',
        });
    },
    ...
    markAllAsRead: async () => {
        return await apiCall(`${API_ENDPOINTS.notifications}mark_all_as_read/`, {
            method: 'POST',
        });
    },
};
...
/**
 * API Messages (extension de ContactAPI pour les réponses)
 */
const MessagesAPI = {
    // GET /api/contact/mes_messages/ - Mes messages avec réponses
    getMesMessages: async () => {
        return await apiCall(`${API_ENDPOINTS.contact}mes_messages/`);
    },
    
    // POST /api/contact/{id}/reply/ - Admin répond à un message
    reply: async (id, reponse) => {
        return await apiCall(`${API_ENDPOINTS.contact}${id}/reply/`, {
            method: 'POST',

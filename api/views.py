...
def create_notification(user, notif_type, message, lien=None):
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        for admin in admins:
            create_notification(
                ...
                message=f"Nouvelle commande #{order.id} de {order.customer_name} — {order.total_price} FCFA",
                lien='orders-management'
            )
    ...
    def update_status(self, request, pk=None):
        ...
        if order.user and old_status != new_status:
            status_messages = {
                'paid': 'Votre commande a été confirmée (payée)',
                ...
                'cancelled': 'Votre commande a été annulée par l\'administrateur.',
            }
            notif_msg = status_messages.get(new_status, f"Le statut de votre commande #{order.id} a été mis à jour : {order.get_status_display()}")
            notif_type = 'commande_prete' if new_status == 'ready' else 'commande_statut'
            create_notification(
                user=order.user,
                notif_type=notif_type,
                message=f"Commande #{order.id} : {notif_msg}",
                lien='orders'
            )
    ...
    def cancel(self, request, pk=None):
        ...
        # Notifier les admins si c'est le client qui annule
        if not request.user.is_superuser and order.user:
            admins = User.objects.filter(is_superuser=True)
            for admin in admins:
                create_notification(
                    user=admin,
                    notif_type='commande_annulee',
                    message=f"La commande #{order.id} de {order.customer_name} a été annulée par le client.",
                    lien='orders-management'
                )
...
class ContactMessageViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        for admin in admins:
            create_notification(
                ...
                message=f"Nouveau message de {contact_message.name} : {contact_message.get_subject_display()}",
                lien='messages-management'
            )
    ...
    def reply(self, request, pk=None):
        ...
        Body: { "reponse": "Texte de la réponse" }
        """
        from django.utils import timezone
        
        if not request.user.is_superuser:
            return Response(
                {'error': 'Seuls les administrateurs peuvent répondre aux messages'},
        ...
        contact_message.admin_reponse = reponse
        contact_message.repondu_le = timezone.now()
        contact_message.repondu_par = request.user
        contact_message.status = 'replied'
        contact_message.save()
        
        # Notifier le client si le message est lié à un compte
        if contact_message.user:
            create_notification(
                user=contact_message.user,
                notif_type='reponse_message',
                message=f"L'administrateur a répondu à votre message : \"{contact_message.get_subject_display()}\"",
                lien='contact'
            )
    ...
    def mes_messages(self, request):
        """
        Retourne les messages de l'utilisateur connecté avec les réponses
        """
    ...
    def unread(self, request):
        ...
        """
        messages_qs = ContactMessage.objects.filter(status='new')
        # Filtrer par utilisateur si pas admin
        if not request.user.is_superuser:
            messages_qs = messages_qs.filter(user=request.user)
...
def cancel_order_api(request, order_id):
    """
    API pour annuler une commande (après 1h d'attente depuis la création)
    """
    try:
        ...
        if request.user.is_superuser:
            ...
            # Les clients ne peuvent annuler que leurs propres commandes
            order = Order.objects.get(id=order_id, user=request.user)
        
        # Vérifier si la commande peut être annulée (règle 1h pour les clients)
        if not request.user.is_superuser:
            ...
            # Admin: vérifier seulement que la commande n'est pas déjà annulée ou livrée
            if order.status == 'cancelled':
                return Response({'success': False, 'message': 'Commande déjà annulée'}, status=400)
            if order.status == 'delivered':
                return Response({'success': False, 'message': 'Impossible d\'annuler une commande livrée'}, status=400)

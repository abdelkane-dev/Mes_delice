...
class Order(models.Model):
    ...
    def can_cancel(self):
        """Vérifie si la commande peut être annulée (après 1h d'attente)"""
        from django.utils import timezone
        import datetime
        
        # La commande peut être annulée si elle est en statut pending
        # et qu'au moins 1 heure s'est écoulée
        if self.status != 'pending':
            return False, f"Impossible d'annuler une commande en statut '{self.get_status_display()}'"
        
        time_diff = timezone.now() - self.created_at
        hours_passed = time_diff.total_seconds() / 3600
        
        if hours_passed < 1:
            return False, f"Veuillez attendre {1 - hours_passed:.1f} heures avant de pouvoir annuler cette commande"
        
        return True, "La commande peut être annulée"
    ...
    def cancel_order(self):
        ...
        if self.status != 'cancelled':
            ...
            # Enregistrer le montant remboursé
...
class ContactMessage(models.Model):
    ...
    # Champs pour la réponse admin
    admin_reponse = models.TextField(
        blank=True,
        null=True,
        verbose_name="Réponse de l'admin"
    )
    repondu_le = models.DateTimeField(
        blank=True,
        null=True,
    ...
    @property
    def est_repondu(self):
        """Indique si le message a reçu une réponse"""
        return bool(self.admin_reponse)

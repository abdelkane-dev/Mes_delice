...
urlpatterns = [
    ...
    path('', login_view, name='login'),  # Page d'accueil = LOGIN (GET/POST)
    path('client/', client_view, name='client_view'),  # Page client
    path('management/', admin_view, name='admin_view'),  # Page admin
    path('management/users/', users_management_view, name='users_management'),  # Gestion utilisateurs
    path('register/', register_view, name='register'),  # Inscription client
    path('logout/', logout_view, name='logout'),  # Déconnexion
]

# Servir les fichiers media toujours (dev et production)
# WhiteNoise gère le static, Django sert le media directement
